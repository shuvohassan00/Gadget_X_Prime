...
def normalize_public_url(url: str) -> str:
    ...
    return resolve_short_url(cleaned)


def resolve_short_url(url: str) -> str:
    """
    Resolve common short/share links to their final destination URL.
    This helps yt-dlp when platforms provide temporary redirect wrappers.
    """
    target = (url or "").strip()
    ...
    try:
        ...
        with urlopen(req, timeout=12) as resp:
            final_url = getattr(resp, "url", "") or target
        return final_url
    except Exception:
        try:
            req = Request(
            ...
            with urlopen(req, timeout=12) as resp:
                return getattr(resp, "url", "") or target
...
def pick_video_qualities(url: str) -> List[Dict[str, Any]]:
    ...
    for item in qualities:
        ...
        trimmed.append(item)
    if not trimmed:
        # Fallback list for platforms that do not provide rich format metadata.
        return [
            {"label": "Best", "height": 0, "format_id": "best", "ext": "mp4", "filesize": None, "has_audio": True},
            {"label": "Good", "height": 0, "format_id": "bv*[height<=720]+ba/b[height<=720]/best", "ext": "mp4", "filesize": None, "has_audio": True},
        ]
    return trimmed[:8]
...
def download_video(source: str, format_id: Optional[str], max_height: Optional[int], work_dir: Path) -> Dict[str, Any]:
    ...
    if format_id:
        format_selector = f"{format_id}/best[format_id={format_id}]/best"
    elif max_height:
        format_selector = (
    ...
    with yt_dlp.YoutubeDL(opts) as ydl:
        try:
            ...
            # Fallback for non-YouTube platforms where explicit selectors fail.
            fallback_opts = opts | {"format": "best", "merge_output_format": "mp4"}
            with yt_dlp.YoutubeDL(fallback_opts) as ydl_fb:
                info = resolve_ydl_entry(ydl_fb.extract_info(source, download=True))
